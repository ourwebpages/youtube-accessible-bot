...
class VideoRepository:
    ...
    def search(self, user_id: int, query: str, limit: int = 10):
        with self._connect() as conn:
            return conn.execute(
                """SELECT * FROM videos
                WHERE user_id=? AND (title LIKE ? OR channel LIKE ? OR transcript LIKE ?)
    ...
    def favorites(self, user_id: int):
        with self._connect() as conn:
            return conn.execute(
                "SELECT * FROM videos WHERE user_id=? AND favorite=1 ORDER BY created_at DESC",
                (user_id,),
            ).fetchall()
    ...
    def set_favorite(self, user_id: int, video_id: str, value: bool = True) -> None:
        with self._connect() as conn:
            conn.execute(
                "UPDATE videos SET favorite=? WHERE user_id=? AND video_id=?",
                (int(value), user_id, video_id),
            )

    def toggle_favorite(self, user_id: int, video_id: str) -> bool:
        with self._connect() as conn:
            row = conn.execute(
                "SELECT favorite FROM videos WHERE user_id=? AND video_id=?",
                (user_id, video_id),
            ).fetchone()
            if row is None:
                return False
            ...
            conn.execute(
                "UPDATE videos SET favorite=? WHERE user_id=? AND video_id=?",
                (int(new_value), user_id, video_id),
            )
            return new_value
